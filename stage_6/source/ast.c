...
{
    if (nodeType == CONNECTOR_NODE)
        return "CONNECTOR_NODE";
    else if (nodeType == READ_NODE)
        return "READ_NODE";
    else if (nodeType == WRITE_NODE)
        return "WRITE_NODE";
    else if (nodeType == PLUS_OP_NODE)
        return "PLUS_OP_NODE";
    else if (nodeType == MINUS_OP_NODE)
        return "MINUS_OP_NODE";
    else if (nodeType == MUL_OP_NODE)
        return "MUL_OP_NODE";
    else if (nodeType == DIV_OP_NODE)
        return "DIV_OP_NODE";
    else if (nodeType == MOD_OP_NODE)
        return "MOD_OP_NODE";
    else if (nodeType == LT_OP_NODE)
        return "LT_OP_NODE";
    else if (nodeType == LT_EQ_OP_NODE)
        return "LT_EQ_OP_NODE";
    else if (nodeType == GT_OP_NODE)
        return "GT_OP_NODE";
    else if (nodeType == GT_EQ_OP_NODE)
        return "GT_EQ_OP_NODE";
    else if (nodeType == EQUAL_EQ_OP_NODE)
        return "EQUAL_EQ_OP_NODE";
    else if (nodeType == NOT_EQ_OP_NODE)
        return "NOT_EQ_OP_NODE,";
    else if (nodeType == AND_NODE)
        return "AND_NODE";
    else if (nodeType == OR_NODE)
        return "OR_NODE";
    else if (nodeType == EQ_OP_NODE)
        return "EQ_OP_NODE";
    else if (nodeType == IF_NODE)
        return "IF_NODE";
    else if (nodeType == BRANCH_NODE)
        return "BRANCH_NODE";
    else if (nodeType == WHILE_NODE)
        return "WHILE_NODE";
    else if (nodeType == BREAK_NODE)
        return "BREAK_NODE";
    else if (nodeType == CONTINUE_NODE)
        return "CONTINUE_NODE";
    else if (nodeType == IDENTIFIER_NODE)
        return "IDENTIFIER_NODE";
    else if (nodeType == FIELD_NODE)
        return "FIELD_NODE";
    else if (nodeType == INTEGER_NODE)
        return "INTEGER_NODE";
    else if (nodeType == STRING_NODE)
        return "STRING_NODE";
    else if (nodeType == ARRAY_NODE)
        return "ARRAY_NODE";
    else if (nodeType == INDEX_NODE)
        return "INDEX_NODE";
    else if (nodeType == DEREF_NODE)
        return "DEREF_NODE";
    else if (nodeType == ADDR_OF_NODE)
        return "ADDR_OF_NODE";
    else if (nodeType == FUNCTION_CALL_NODE)
        return "FUNCTION_CALL_NODE";
    else if (nodeType == RETURN_NODE)
        return "RETURN_NODE";

    return "UNKNOWN_NODE_TYPE";
}

struct ASTNode *CreateASTNode(int val, char *varName, int nodeType, int expTypeIndex, struct ASTNode *left, struct ASTNode *right)
{
    ...
    node->expTypeIndex = expTypeIndex;
    node->left = left;
    node->right = right;
    node->symbol = 0;
    node->argList = 0;
    ...
    if (nodeType == IDENTIFIER_NODE)
    {
        ...
        {
            node->symbol = symbol;
            node->expTypeIndex = symbol->typeIndex;
        }
    }
...
void GenerateInitMemCode(int reg, FILE *output)
{
    int a = GetRegister();

    fprintf(output, "\n");
    fprintf(output, "MOV R%d, \"Heapset\"\n", a);
    fprintf(output, "PUSH R%d\n", a);
    fprintf(output, "PUSH R%d\n", a);
    fprintf(output, "PUSH R%d\n", a);
    fprintf(output, "PUSH R%d\n", a);
...
void GenerateAllocMemCode(int reg, FILE *output)
{
    int a = GetRegister();
...
void GenerateFreeMemCode(int reg, FILE *output)
{
    int a = GetRegister();
...
int GenerateCode(struct ASTNode *node, FILE *output)
{
    ...
    case READ_NODE:
    {
        ...
        if (localSymbol)
        {
            ...
            GenerateReadCodeReg(tmp, output);
            FreeRegister();
            return 0;
        }
    ...
    {
        int reg = GetRegister();
        GenerateInitMemCode(reg, output);
        return reg;
    }
    break;
    ...
    {
        int reg = GetRegister();
        GenerateAllocMemCode(reg, output);
        return reg;
    }
    break;
    ...
    {
        int reg = GenerateCode(node->left, output);
        GenerateFreeMemCode(reg, output);
        return reg;
    }
    break;
    ...
    {
        int reg = GetRegister();
        int typeIndex = CheckSemantics(node->left);
        int index = LookUpFieldIndex(typeTable.types[typeIndex].fieldList, node->right->varName);
    ...
    case EQ_OP_NODE:
    {
        ...
        else if (node->left->nodeType == FIELD_NODE)
        {
            int typeIndex = CheckSemantics(node->left->left);
            int index = LookUpFieldIndex(typeTable.types[typeIndex].fieldList, node->left->right->varName);
            int tmp = GenerateCode(node->left->left, output);
            fprintf(output, "ADD R%d, %d\n", tmp, index);
            int reg = GenerateCode(node->right, output);
            fprintf(output, "MOV [R%d], R%d\n", tmp, reg);
            FreeRegister();
    ...
    case FUNCTION_DEF_NODE:
    {
        ...
        if (node->localSymbolTable)
        {
            ...
            {
                if (localSymbolTable.symbols[n].binding >= 1)
                    localVarCount++;
            }
...
void PrintAST(struct ASTNode *node, int indent)
{
    ...
    case FUNCTION_DEF_NODE:
    {
        printf("FUNCTION DEF: %s %s( ", TypeToString(node->returnTypeIndex), node->varName);
        if (node->paramList)
        {
            for (int n = 0; n < node->paramList->size; n++)
            {
                printf("%s %s, ", TypeToString(node->paramList->params[n].typeIndex), node->paramList->params[n].name);
            }
        }
        printf(")\n");
...
int CheckSemantics(struct ASTNode *node)
{
    ...
    {
        // int typeIndex = CheckSemantics(node->left);
        // if (LookUpTypeTableIndex(typeTable, "int") != typeIndex && LookUpTypeTableIndex(typeTable, "str") != typeIndex)
        // {
        //     // printf("error: 'write()' library call expects argument of type 'int' or 'str' but found '%s'\n", typeTable.types[typeIndex].name);
        //     // exit(1);
        // }
    }
    break;
    ...
    {
        int typeIndex = CheckSemantics(node->left);
        if (LookUpTypeTableIndex(typeTable, "int") != typeIndex)
        {
            printf("error: 'read()' library call expects argument of type 'int' but found '%s'\n", typeTable.types[typeIndex].name);
            exit(1);
        }
    }
    break;
    ...
    case INIT_MEM_NODE:
    {
        return LookUpTypeTableIndex(typeTable, "int");
    }
    break;
    ...
    case FREE_MEM_NODE:
    {
        ...
        if (IsPrimitiveType(typeTable, typeIndex))
        {
            printf("error: 'free()' library call expects argument of user defined type but found primitive type '%s'\n", typeTable.types[typeIndex].name);
            exit(1);
        }
    }
    break;
    ...
    case STRING_NODE:
    {
        return LookUpTypeTableIndex(typeTable, "str");
    }
    break;

    case NULL_NODE:
    {
        return LookUpTypeTableIndex(typeTable, "null");
    }
    break;
    ...
    case IDENTIFIER_NODE:
    {
        ...
        if (localSymbol)
        {
            node->expTypeIndex = localSymbol->typeIndex;
            return localSymbol->typeIndex;
        }
        ...
        if (node->symbol)
        {
            ...
            {
                node->expTypeIndex = node->symbol->typeIndex;
                return node->symbol->typeIndex;
            }
            else
            {
    ...
    case FIELD_NODE:
    {
        int index = CheckSemantics(node->left);
        struct Field *field = LookUpField(typeTable.types[index].fieldList, node->right->varName);

        if (field)
        {
            return field->typeIndex;
        }

        printf("error: struct '%s' has no member named '%s'\n", typeTable.types[index].name, node->right->varName);
        exit(1);
    }
    ...
    case ARRAY_NODE:
    {
        ...
        int index = LookUpTypeTableIndex(typeTable, "int");
    ...
    case EQ_OP_NODE:
    {
        ...
        {
            int nullTypeIndex = LookUpTypeTableIndex(typeTable, "null");

            if (rightExprType == nullTypeIndex)
            {
                if (IsPrimitiveType(typeTable, leftExprType))
                {
                    printf("error: type mismatch in assignment statement!\n");
                    printf("note: lhs type = '%s'\n", typeTable.types[leftExprType].name);
                    printf("note: rhs type = '%s'\n", typeTable.types[rightExprType].name);
                    exit(1);
                }
            }
        }
        node->expTypeIndex = leftExprType;
    }
    break;
    ...
    case NOT_EQ_OP_NODE:
    {
        ...
        if (rightExprType == nullTypeIndex && !IsPrimitiveType(typeTable, leftExprType))
        {
            return LookUpTypeTableIndex(typeTable, "bool");
        }
    ...
    case OR_NODE:
    {
        ...
        int index = LookUpTypeTableIndex(typeTable, "bool");
    ...
    case DEREF_NODE:
    {
        ...
        {
            node->expTypeIndex = INTEGER_TYPE;
            return INTEGER_TYPE;
        }

        if (type == POINTER_STR_TYPE)
        {
            node->expTypeIndex = STRING_TYPE;
            return STRING_TYPE;
        }
    ...
    case FUNCTION_CALL_NODE:
    {
        ...
        if (symbol.paramList && node->argList)
        {
            for (int n = 0; n < node->argCount; n++)
            {
                ...
                int expectedType = symbol.paramList->params[n].typeIndex;
                if (argType != expectedType)
                {
                    printf("error: incompatible type for argument %d of function '%s'\n", n, node->left->varName);
                    printf("note: expected '%s' but argument is of type '%s'\n", typeTable.types[expectedType].name, typeTable.types[argType].name);
                    exit(1);
                }
            }
        }
    ...
    case RETURN_NODE:
    {
        ...
        if (type != returnTypeIndex)
        {
            printf("error: incompatible return type expected '%s' but found '%s'\n", typeTable.types[returnTypeIndex].name, typeTable.types[type].name);
            exit(1);
        }
    }
    break;
    ...
    case FUNCTION_DEF_NODE:
    {
        ...
        else
        {
            ...
            if (symbol->typeIndex != node->returnTypeIndex)
            {
            ...
            if (symbol->paramList && node->paramList)
            {
                ...
                for (int n = 0; n < node->paramList->size; n++)
                {
                    if (symbol->paramList->params[n].typeIndex != node->paramList->params[n].typeIndex)
                    {
        ...
        returnTypeIndex = node->returnTypeIndex;
