...
void InstallGlobalSymbol(struct GlobalSymbolTable *table, struct GlobalSymbol symbol)
{
    ...
    table->staticAddressOffset += symbol.size;
...
{
    if (typeIndex == LookUpTypeTableIndex(typeTable, "int") ||
        typeIndex == LookUpTypeTableIndex(typeTable, "str") ||
        typeIndex == LookUpTypeTableIndex(typeTable, "bool"))
    {
        return true;
    }
